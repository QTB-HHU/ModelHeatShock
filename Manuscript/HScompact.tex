...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \maketitle
    \begin{abstract}
Global warming exposes plants to severe heat stress, with consequent crop yield reduction. Organisms exposed to high temperature stresses typically protect themselves with a heat shock response (HSR), where accumulation of unfolded proteins initiates the synthesis of heat shock proteins through the heat shock transcription factor HSF1. While the molecular mechanisms are qualitatively well characterized, our quantitative understanding of the underlying dynamics is still very limited. Here we study the dynamics of HSR in the photosynthetic model organism \emph{Chlamydomonas \textcolor{blue}{reinhardtii}} with a data driven mathematical model of HSR. \textcolor{blue}{We based our dynamical model mostly on mass-action kinetics}, with a few non-linear terms\textcolor{blue}{. We parameterized it by experimental data-sets and validated it} on several independent data-sets\textcolor{blue}{, all from the literature}. By comparing the HSRs in laboratory conditions (abrupt high temperature increases) to typical natural conditions (smooth temperature variations) we show that the HSR in \emph{Chlamydomonas reinhardtii} is well adapted to natural conditions in which cells accumulate only negligible amounts of misfolded proteins. We demonstrate that HSR quantitatively and significantly differs if an increase in temperature of the same magnitude occurs in natural or in laboratory conditions. The mathematical model we developed is a flexible tool to simulate the HSR to different conditions and complements the current experimental approaches. 
...
From experiments performed by Schmollinger et al. \cite{Schmollinger2013}, \textcolor{blue}{which led to hypothesizing the signalling mechanisms shown in Fig. 11 therein, we derive the signalling network schematically depicted in Fig.~\ref{Figure1label}~A performing the above simplifications and adding the description of the genome transcription. This signalling network represents the base to build our mathematical model of the HSR.} %All components are described in detail in Table~\ref{TabVars}. 
...
most of the twenty rate constants listed in Supplementary Table~\ref{TabKs}  are not explicitly known although for some of them a reasonable range can be estimated from data bases (\cite[BNID 103916, 106521, 105408]{Milo2010}). %This again
...
\textcolor{blue}{It is thus important to keep in mind that the calibration data we used, which are relative only to two species, namely $mR_{F}$ and $mR_{HP}$, are not sufficient to identify the model parameters uniquely, nor to provide estimates of these parameters independently of the other parameters' values. } %, but in any case this small RMS would not be that much smaller than the value corresponding to the fiducial parameter set. 
...
\textcolor{blue}{To investigate these correlations further, in Supplementary Material \ref{SecCalibrationExtended}, we also investigate pair-wise relationships among model parameters, as illustrated in Supplementary Material~Fig.~\ref{Figure6label}. This limit of the calibration procedure makes very important the accurate construction of the model structure, which as previously stated we based on the experimental findings of \cite{Schmollinger2013}.}
...
The HSR that we model is not a circadian clock, and has no oscillatory behaviour on its own\textcolor{blue}{. We nevertheless observe from Fig.~\ref{Figure4label}~(B-F) that this HSR is activated just before dawn, i.e. at around 6:00 a.m., when the concentration of $P^\#$ shown in Fig.~\ref{Figure4label}~panel~A becomes enough to trigger the HSR. Then, the intensity of the HSR reaches a maximum about one hour and a half before the maximum of the temperature, which occurs at 15:00. 
%, Fig.~\ref{Figure4label}~panel~G). 
It then drops slowly during the rest of the day, it is roughly off when night comes, and it stays off over night. We indeed observe in Fig.~\ref{Figure4label}~(A-F) that, as soon as the temperature increase has led to the accumulation of enough $P^\#$ to trigger an HSR, this HSR is strong enough not only to handle the currently present amount of $P^\#$, but also to prevent the subsequent temperature increase (which still continue for hours) to lead to the increase of $\left[ P^\# \right]$ above the level which triggered the initiation of the HSR. This allows the cell to remain exposed to high temperatures for several hours during the day 
%as shown in Fig.~\ref{Figure4label}~panel~G, 
accumulating at maximum a concentration of $P^\#$ remarkably smaller than the concentration reached when the temperature increase occurs within minutes instead of hours, as already underlined. This observation may lead to speculate and propose the hypothesis that the HSR might be adapted to daily temperature variations rather than abrupt temperature changes. The system's parameters could thus be the result of adaptation to variable thermal environments, such that the HSR would be well suited to handle daily temperature fluctuations.}
...
Moreover the parameter set which provides the best RMS among this randomly generated \textcolor{blue}{sets (i.e. the isolated point at the very bottom of each panel in Fig.~\ref{Figure2label}~C), when used in the model to simulate other situations as for instance the double heat shock experiment of Fig.~\ref{Figure3label}~C, provide to the model a behaviour which fails to capture key features that the real system has shown in the experiments, in this case for instance the capability of showing a second HSR of the same amplitude of the first one only if the two consecutive HSs occur at least 5h of distance. Furthermore, other parameter sets providing a low RMS in the MC approach were providing oscillations in the concentration of $mR_{F}$ and $mR_{HP}$ which are not observed in the real system. We indeed considered to include the double heat shock experiment data into the calibration procedure, but this would require some higly arbitrary steps as described in Supplementary Material~\ref{sectionRMS2HS} and thus we discarded that approach.}
