...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \maketitle
    \begin{abstract}
Global warming is exposing plants to more severe heat stress, with consequent crop yield reduction. 
...
The dynamical model is mostly based on mass-action kinetics with a few non-linear terms inferred from literature, parameterized by several experimental data sets and validated by independent test data. Comparing HSRs in typical lab conditions with abrupt high temperature increases to more natural, smooth temperature variations shows that the HSR  in \emph{Chlamydomonas reinhardtii} is adapted to natural conditions in which cells accumulate only negligible amounts of misfolded proteins.
...
These concentrations are expressed in arbitrary units (a.u.) and normalized to a standard value for each panel, as summarized in Supplementary Table~\ref{TabRefVal}, allowing for direct comparisons with relative values from experiments.
...
with time course data of the HP concentration from~\cite{Muehlhaus2011}. 
