...
\textcolor{blue}{the twenty rate constants listed in Supplementary Table~\ref{TabKs} are not explicitly known although for few of them a reasonable range can be estimated from data bases. In particular, we ensured to keep the rate constants $k_P$, $k_{FG}$ and $k_{F^*G}$ in a range lower than the diffusion controlled limit of enzymatic reaction rate estimated 
% to be within $10^2$ and $10^3$ $(\mu M s)^{-1}$ (
in \cite[BNID 103916]{Milo2010}. We further select a value for $k_{F^*G}$ with the same order of magnitude of the activator association rate reported in (\cite[BNID 106521]{Milo2010}). 
%(\cite[BNID 103916, 106521, 105408]{Milo2010}). 
Keeping this information into consideration, we first manually tuned all the parameters to reproduce the qualitative behaviour of the experimental data.} 
%This again
...
to the first heat shock occurs later than in the control experiments used to calibrate the parameter sets (see~\ref{Figure2label}~A,B). 
