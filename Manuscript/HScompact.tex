...
\title{Data-driven dynamical model shows that the heat shock response in \emph{Chlamydomonas reinhardtii} is adapted to the natural temperature variation}

\author[1,2]{Stefano Magni}
\author[1,3]{Antonella Succurro}
\author[2]{Alexander Skupin}
\author[4,$\dagger$]{Oliver Ebenhöh}
...
\affil[4]{\small{Cluster of Excellence on Plant Sciences (CEPLAS), Institute for Quantitative and Theoretical Biology, Heinrich Heine University Düsseldorf, Germany}}
...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \maketitle
    \begin{abstract}
Global warming is exposing plants to more frequent heat stress, with consequent crop yield reduction. 
Organisms exposed to large temperature increases protect themselves typically with a heat shock response (HSR). 
%We have built  
To study the HSR in photosynthetic organisms we present here %in this work 
a data driven mathematical model describing the dynamics of the HSR in 
%AS %a photosynthetic model organism. 
the model organism \emph{Chlamydomonas reinhartii}. 
Temperature variations are sensed by the accumulation of unfolded proteins, %AS
which activates the synthesis of heat shock proteins (HSPs) 
mediated by the heat shock transcription factor HSF1. 
Our dynamical model employs a system of ordinary differential equations mostly based on mass-action kinetics, with the addition of some other non-linear terms, to study the time evolution of the involved species. 
The signalling network is inferred from data in the literature, and a part of the multiple experimental data-sets available is used to calibrate the model, while the other part is employed to show that the model is able to reproduce their qualitative behaviour. 
With 
this model 
...
However, while this has been %investigated by means of a 
considerably investigated experimentally, the complementary theoretical activities %involving mathematical modelling 
until now have been rather limited. In this work we thus contribute to fill this gap by developing a mathematical model of the heat shock response in the photosynthetic model organism \emph{Chlamydomonas reinhardtii}.
...
That model mainly focuses on the auto-regulatory mechanism involving HSF1 and HSP90, % (see next section), 
but it does not include a detailed transcriptional regulation. 
...
In Section~\ref{SectionModel0} we introduce the signalling network used to implement the mathematical description of the HSR, we discuss the typical behaviour of the model and we explain how we 
...
calibrate it to data from literature. 
In Section~\ref{SectionExperiments} we compare simulation results with additional data from the literature,
...
In Section~\ref{SecHotDay} we employ the model to simulate interesting conditions that have not yet been 
...
systems dynamics and its emergent properties, before concluding in Section~\ref{SectionConclusions}. 
...
The temporal dynamics of the variables are governed by a set of ordinary differential equations (ODEs) reported in Table~\ref{TabODEs} of the Supplementary Material. 
...
in the wide range reported by \cite{Bischof2006,He2003}. A detailed description of the mathematical model can be found in Section~\ref{Tables} of the Supplementary Material.
...
When present, a red background indicates the period of time during which a heat shocking temperature is provided. 
...
functional proteins are mis-folded (panel~A). 
...
We initially considered biologically reasonable ranges 
...
in other experiments. The group of data used for calibration comprises the controls for the feeding
...
for HSF1, and six for the mRNA coding for HSP90A, under HS and no inhibitor treatment. These data are shown in Fig.~\ref{2D}. 
%These
...
and we show in Fig.~\ref{Figure1} (for each of the 20 parameters separately) the values of the RMS versus the value of each parameter. We observe that for the vast majority of the parameters no preferred interval in which the lowest values of the RMS occur more often can be identified. % Very low values of the RMS can occur almost everywhere in the interval used for the random scan. 
...
This observation lead us to conclude that many different configurations in the parameter space would allow us to obtain a small RMS with respect to the data, but in any case this small RMS would not be that much smaller than the value corresponding to the fiducial parameter set. 
...
As shown in Fig.~\ref{GradientSearch_RMS} our algorithm stopped after several iterations and returned the set of parameters listed in the third column of Table~\ref{TabKs} of the Supplementary Material as final values. The corresponding value of the RMS w.r.t. the controls of the feeding experiments is $0.137$. 
...
is provided in Section \ref{SecCalibrationExtended} of the Supplementary Material. 
...
produce a plausible HSR, but can also quantitatively (in relative terms) reproduce %(relative, not absolute on which we do not have information) 
a number of key experimental treatments. 
...
to identify the minimum relaxation time needed to observe a full second response. We finally reproduce in Section~\ref{SecTimeCurse} the observed HP expression from \cite{Muehlhaus2011}.
%These experiments were used for the fine tuning of the parameters. The 
%calibration procedure is described in Section~\ref{SecCalibration}.


%In this section we show that the model is able to reproduce the
%qualitative behaviour of the HSR observed in experiments and it
%provides responses which are in a reasonable concentration range and
%with time scales that fit the experiments.
%
%We do so by considering different data sets available in the literature and comparing the simulations performed with our model to what is experimentally observed. %We will subsequently explain how we have used the data for calibration and validation of the model.
%%The model capability of reproducing the behaviour observed in experiments is used to validate it. 
%The model's parameters have biologically reasonable values and have been calibrated from the controls data of the feeding experiments performed in \cite{Schmollinger2013}, as described in section \ref{SecCalibration} and in the Supplementary Material. 
...
Thus, the model simulations allow to give a novel interpretation of
...
Unlike in section \ref{SecFeeding}, where comparisons among model's predictions and experimental data are at the level of mRNA production, here we would like to compare with data for the time evolution of the concentration of HP from \cite{Muehlhaus2011}. 
...
To simulate this we apply at t=$0$ min an HS by increasing (as in the experiments) the temperature from 25°C to 45°C. We compared the model simulated behaviour of HP with the data relative to HSP70A, HSP70B and HSP90.  

Fig.~\ref{FigTimeCoruseData} shows that the model reproduces well the qualitative behaviour of the data:
even if we can notice a somewhat faster increase during the first 100 minutes of the model's prediction w.r.t. the data, the concentration of HP increases slowly at the very beginning, then faster, reaching a maximum and mildly decreasing afterwards, in the simulation as in the experiments.  Quantitatively, the data are provided in terms of the z-score, 
%a quantity used in statistic to describe a set of data points, which 
measuring the distance of the single data point from the mean, in terms of standard deviations. Since the information needed to convert this scale to a scale which would allow direct comparison to our results is not availble, in Fig. we superpose our simulation results and the data and clearly indicate on the two sides of the figure the different scales. 
...
of the heat shock response, but which are either difficult to test or have not yet been tested experimentally. Further such scenarios are explored in Section \ref{SectionFurtherPredictions} of the Supplementary Material.
...
Thus, which kind of HS is a \emph{C. reinhardtii} cell going to experience in the wild? %If, on the one hand, for laboratory purposes 
...
%\section{Conclusions}
%\label{SectionConclusions}
In this work, we have built a data driven mathematical model for the HSR in \emph{Chlamydomonas reinhartii}, a photosynthetic model organism. We have extracted the signalling network structure from various experiments, and experimental data are used for calibration of the model, as described in Section~\ref{SectionModel0}.
...
In Section~\ref{SectionExperiments} we have compared the model behaviour with further experimental results extracted from the literature and not employed for the calibration: 
...
We have finally studied how the maximal concentration of unfolded proteins reached during HS depends on the time taken by the temperature to increase from the minimal to the maximal value (Fig.~\ref{UnfoldedPfuncOfTau}), and shown that indeed for any time shorter than about 1 minute (i.e. a sharp increase in temperature) the maximal concentration of unfolded proteins is about twenty times higher than the value it assumes for any time longer than about 100 minutes. This further confirms that the HSR of \emph{Chlamydomonas reinhartii} is able to keep the concentration of unfolded proteins at much lower levels (about twenty times) if the increase in temperature occurs on a time scale of the order of hundred minutes (as it is more frequent in nature) rather than in the case of the sharp increase in temperature usually artificially provided during experiments. %These findings are of rather general validity for plants due to the conservation of the HSR among species. 
...
This shows that the HSR of \emph{Chlamydomonas reinhartii} is indeed adapted to natural temperature variations.
